# set -euox pipefail
set -euo pipefail

./run_sphinx_copy.sh

# for project in octoprobe tentacle testbed_heatguard usbhubctl
for project in octoprobe tentacle octohub4 testbed_heatguard testbed_micropython
do
    docs_dir=~/work_octoprobe/${project}/docs
    html_dir=${docs_dir}/_build/html
    if [ -d "$docs_dir" ]; then
        echo "Directory upload: $html_dir"
        make -C $docs_dir clean
        make -C $docs_dir html
        tar cf - -C $html_dir  --transform "s,^\.,${project}," . | ssh www-insecure@www.maerki.com tar xf - -C /home/www/htdocs_insecure/octoprobe
    else
        echo "Directory does not exist: $docs_dir"
    fi
done